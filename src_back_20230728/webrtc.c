...
int rtc_sess_free(rtc_sess_t *rtc_sess)
{
  ...
  osd->point[0] = 10;
  osd->point[1] = 10;
  sprintf(osd->text, "conneted: %d", sess_cnt);
  warn("%s\n", osd->text);
...
rtc_sess_t *__rtc_sess_new()
{
  ...
  strcpy(conf_.iceServers[0].urls, "stun:stun.l.google.com:19302");
  strcpy(conf_.iceServers[1].urls, "stun:stun1.l.google.com:19302");
...
rtc_sess_t *rtc_sess_new()
{
  ...
  {   
    ...
    list_del(&rtc_sess->list);// rtc_sess is init before,save in a list,if rtc_cess is using now,del from the init list
    sess_cnt++;
  }
